...
class GLVideoOutputImpl extends GLVideoOutput {
    ...
    protected void onOutputPrepare(MediaFormat inputFormat) {
        LogUtils.i(TAG, "onOutputPrepare " + inputFormat);
        ...
        LogUtils.i(TAG, "onOutputPrepare bufferMode:" + bufferMode);
    ...
    public synchronized void setTextureSource(@TextureSource int textureSource) {
        LogUtils.i(TAG, "setTextureSource " + textureSource);
        if (isPlayerPrepared()) {
    ...
    public synchronized void setHdrBitDepth(@HdrBitDepth int hdrDisplayBitDepth) {
        LogUtils.i(TAG, "setHdrBitDepth " + hdrDisplayBitDepth);
        if (isPlayerPrepared()) {
    ...
    public synchronized void addVideoTransform(GLVideoTransform videoTransform) {
        LogUtils.i(TAG, "addVideoTransform " + videoTransform);
    ...
    protected void onOutputFormatChanged(MediaFormat outputFormat) {
        ...
        if (bufferMode) {//用buffer转纹理
            ...
            LogUtils.i(TAG, "onOutputFormatChanged " + textureY2YMode);
            if (textureY2YMode) {
...
//            y2yExtTextureRenderer.setScale(scale);
    ...
    protected void onOutputBufferAvailable(ByteBuffer outputBuffer, long presentationTimeUs) {
        ...
        if (bufferMode) {//输入buffer
            LogUtils.i(TAG, "onOutputBufferAvailable: ");
            bufferYUV420Renderer.updateBuffer(outputBuffer);
        } else {
            LogUtils.i(TAG, "onOutputBufferAvailable: is null");
        }
    }
    ...
    protected synchronized boolean onOutputBufferRender(long presentationTimeUs) {
        if (!outputSurface.isValid()) {
            LogUtils.i(TAG, "onOutputBufferRender: !isValid");
            return false;
        }
        LogUtils.i(TAG, "onOutputBufferRender: " + videoSurface.isAttached() + " textureY2YMode:" + textureY2YMode);
        ...
        if (transformList.isEmpty()) {//没有transform直接输出到screen
            LogUtils.i(TAG, "onOutputBufferRender: transformList.isEmpty()");
        ...
        if (windowSurface == null) {
            LogUtils.i(TAG, "onOutputBufferRender: windowSurface is null");
            return false;
        }
        // 把最终的纹理上屏
