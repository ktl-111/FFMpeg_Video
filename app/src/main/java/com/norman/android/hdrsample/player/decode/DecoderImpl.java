...
abstract class DecoderImpl implements Decoder {
    ...
    @Override
    public synchronized void create(String mimeType, FileSource fileSource) {
        if (state != DECODE_UNINITIALIZED &&
                state != DECODE_DESTROY) {
            return;
        }
        state = DECODE_CREATE;
        onCreate(mimeType, fileSource);
    }
    ...
    protected abstract void onCreate(String mimeType, FileSource fileSource);
