...
public class FileUtils {
    ...
    public static void writeBytes(byte[] array, String fileName) {
        ...
        try {
            ...
            writer = new FileOutputStream(App.Companion.getApplication().getExternalCacheDir() + "/" + fileName + ".h264", true);
            writer.write(array);
            writer.write('\n');
    ...
    public static String writeContent(byte[] array, String fileName) {
        ...
        try {
            // 打开一个写文件器，构造函数中的第二个参数true表示以追加形式写文件
            writer = new FileWriter(App.Companion.getApplication().getExternalCacheDir() + "/" + fileName + ".txt", true);
