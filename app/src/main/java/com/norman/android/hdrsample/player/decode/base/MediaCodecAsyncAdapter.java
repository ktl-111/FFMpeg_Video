 ...
 * 异步MediaCodec
 */
public class MediaCodecAsyncAdapter extends MediaCodec.Callback implements DecodecApi {
    ...
    private String TAG = "MCDecode";

    public synchronized void setOutputSurface(Surface surface) {
        LogUtils.i(TAG, "setOutputSurface: " + surface);
        if (isReleased()) {
            return;
        }
    ...
    public void onOutputFormatChanged(@NonNull MediaCodec codec, @NonNull MediaFormat format) {
        String infoBuilder = "mediacodec onOutputFormatChanged" + "\n" +
                "mediaFormat->" + format + "\n";
        LogUtils.i(TAG, "onOutputFormatChanged");
        LogUtil.d(infoBuilder);
        if (isDirtyCallback()) {
            return;
        }
     ...
     */
    public static final class HolderSurface extends GLTextureSurface {
